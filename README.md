...
- **完整性校验** — 强制验证 HTTP 206 状态码与 `Content-Range` 头，块下载完后校验实际字节数，合并时再次校验每块与总大小；任何环节不符都会删除该块数据并重试，杜绝静默产出损坏文件
- **块级失败隔离** — 单块反复失败累计 5 次后自动跳过，worker 不退出继续下载其他块；最终若有不可恢复的块则整体报错并列出失败块编号，已下载的字节全部保留供下次续传
- **断点续传** — 下载中断后重新运行，自动跳过已完成的块，未完成的块从断点续传；已存在且大小严格相符的块直接跳过，大小不符的会自动清理重下
- **速度估算修正** — 重跑预载入已完成字节后自动重置 ETA 估算，避免显示"先大后小"的速度抖动
...
续传时的处理逻辑：
- 文件大小**严格等于**块预期大小的，标记为已完成并跳过
- 文件大小**大于**预期大小的（通常是上次下载响应异常导致），删除文件从头重下
- 文件大小**小于**预期大小的（上次中断时只写了一部分），通过 Range 头从断点续传
- 速度估算会自动重置，避免预载入已完成字节时显示"先大后小"的虚假速度

下载状态保存在输出文件同目录下的隐藏目录 `.filename/` 中，下载完成后自动清理。
...
4. 检查隐藏目录，扫描已完成的块（断点续传）
   └── 仅接受文件大小严格等于 expected_size 的块为已完成
5. 创建调度器，启动 N 个工作线程
   ├── 从调度器获取一个未占用的块（无锁 CAS 抢占）
   ├── 租用代理（或直连）
   ├── GET 请求 + Range 头
   ├── 校验响应：必须 206（或服务器不支持 Range 时 200）
   ├── 校验 Content-Range 头中的 start 与请求的 range_start 一致
   ├── 流式写入隐藏目录中的块文件
   ├── 写完后校验实际大小 == expected_size，不符则删除文件并重试
   ├── 失败时记录块级失败计数、标记代理，租用新代理重试
   ├── 单块失败累计达 5 次，调度器永久跳过该块，worker 继续取下一个
   └── 完成后标记块已完成，更新清单，继续取下一个块
6. 所有可重试块完成后，检查是否有超阈值失败块或 worker panic，有则报错退出
7. 按顺序合并块文件，逐块校验读取字节数，并最终校验合并总大小 == total_size
8. 清理隐藏目录，输出完成信息
```
